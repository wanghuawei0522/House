...
<script language="JavaScript">
  $(function(){  //加载事件
       //添加点击事件  当前使用的是jquery,后期可以改为vue(了解)
       $("#checkButton").click(function(){
            $.post("checkUserName",{"username":$("#inputName").val()},function(data){  //发送异步请求
                 if(data.result==1)
                     $("#showNotice").html("用名存在，不可用").css("color","red");
                else
...
<TABLE class=field>
  ...
  <TR>
    ...
    <TD><INPUT class=text id="inputName" type=text name=name><span id="showNotice"></span><input type="button" id="checkButton" value="检查用户名是否存在"> </TD></TR>
  <TR>
